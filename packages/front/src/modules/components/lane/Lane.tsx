...
export const Lane = observer(() => {
  const { data, isLoading, manageInterval, addInterval, deleteInterval, editInterval } = useLane();
  return (
    <>
      <BodyTemplate.Container>
        <BodyTemplate.Main>
          ...
          {!isLoading && <img src={addIcon} className={style.addIcon} alt="" onClick={() => addInterval()} />}
          {isLoading && <Loader />}
        </BodyTemplate.Main>
